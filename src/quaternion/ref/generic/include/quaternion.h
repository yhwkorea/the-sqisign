 ...
 *
 * Identical external contract to quat_lideal_reduce_basis: `reduced` is a
 * reduced basis of the lideal lattice (columns, denom-scaled coords) and
 * `gram` is the lattice trace form of `reduced` divided by the ideal norm,
 * with diagonal halved and upper-triangular part zeroed.
 *
 * Internally drives reduction via quat_mlll_gram (Lemma 3 width, Gram-based
 * MLLL) instead of the classic quat_lll_core. Used to benchmark / replace
 * the HNF-backed reduction in SQIsign Alg 4 (see Phase 3 plan).
 */
